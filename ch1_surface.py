...
def main():
    ...
    def plot_num_change_func():
        ...
        if plot_num != root_com[plot_root][-1]:
            ...
            if plot_root == 1:
                plot_root = ch1_choose1_surface.main()
                plot_num = root_com[plot_root][0]
            elif plot_root in [3, 4]:
                if ch1_flight1_surface.main():
                    cfg2.choose_ch1.append(1)
                    plot_root = 5
                    plot_num = root_com[plot_root][0]
                else:
                    plot_root = 8
                ...
                if ch1_flight2_surface.main():
                    key = ch1_choose2_surface.main()
                    while key == 0:
                        ch1_flight2_surface.main()
                        key = ch1_choose2_surface.main()
                    if key == 1:
                        cfg2.choose_ch1.append(10)
                    plot_root = 7
                    plot_num = root_com[plot_root][0]
                else:
                    plot_root = 8
...
# 剧情文本读取
text0 = cfg2.text_get("resource/font/text1.txt")
# print(text0)
text1 = [[]]
for item in text0:
    for text in item:
...
# print(text1)
# 剧情图像绘制
image0 = cfg2.image_get("resource/font/image1.txt")
image1 = [[]]
for item in image0:
    if len(item) > 0:
        image1.append(item)
