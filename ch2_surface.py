...
import ch2_choose2_surface
import ch2_choose3_surface
import ch2_flight1_surface
import ch2_flight2_surface
import ch2_flight3_surface
...
def main():
    ...
    def plot_num_change_func():
        ...
        # 节点分支
            ...
            if plot_root == 2:
                plot_root = ch2_choose1_surface.main()
                plot_num = root_com[plot_root][0]
            elif plot_root in [5, 3]:
                if ch2_flight1_surface.main():
                    plot_root = 6
                    plot_num = root_com[plot_root][0]
                else:
                    plot_root = 18
                ...
                plot_root = ch2_choose2_surface.main()
                plot_num = root_com[plot_root][0]
            elif plot_root == 7:
                if ch2_flight2_surface.main():
                    plot_root = 8
                    plot_num = root_com[plot_root][0]
                    cfg2.choose_ch2.append(2)
                else:
                    plot_root = 18
                    plot_num = root_com[plot_root][0]
            elif plot_root == 12:
                cfg2.choose_ch2.append(1)
                plot_root = 13
            elif plot_root == 13:
                if ch2_flight3_surface.main():
                    key = ch2_choose3_surface.main()
                    while key == 0:
                        ch2_flight3_surface.main()
                        key = ch2_choose3_surface.main()
                    if key == 1:
                        plot_root = 14
                    else:
...
# 剧情文本读取
text0 = cfg2.text_get("resource/font/text2.txt")
# print(text0)
text1 = [[]]
for item in text0:
    for text in item:
...
# print(text1)
# 剧情图像绘制
image0 = cfg2.image_get("resource/font/image2.txt")
image1 = [[]]
for item in image0:
    if len(item) > 0:
        image1.append(item)
...
print(root_com)
print(len(root_com))
control = [[1], [2], [2], [6], [13], [6], [7], [10], [12], [10], [11], [12], [13], [14], [15], [16], [17], [0], [0], [0]]
